...
class AppStore extends Component {
  ...
  render() {
    ...
    const filteredAppList = this.getFilteredAppList(searchResults)
        ...
        <ul className="tabItems-Container">
          {tabsList.map(eachTab => (
            <TabItem
              key={eachTab.tabId}
              tabDetails={eachTab}
              clickAppItem={this.clickAppItem}
            />
          ))}
        </ul>

        <ul className="AppList-items-container">
          {filteredAppList.map(eachItem => (
            <AppItem key={eachItem.appId} appItemDetails={eachItem} />
          ))}
        </ul>
      </div>
    )
